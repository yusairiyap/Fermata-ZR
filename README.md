...
* **Private Mode** — an incognito-style mode for the Browser and YouTube tabs: turning it on wipes cookies, cache and site data for a clean slate, and can automatically wipe them again when you turn it off, so nothing lingers and YouTube stops showing personalized recommendations. Extra privacy options (in the spirit of Brave's) let you block trackers/ads and third-party cookies, and "Always use Private Mode" keeps it on by default, even across app restarts. Toggle it from the toolbar, the secondary/tertiary floating buttons, or the Browser/YouTube menu
